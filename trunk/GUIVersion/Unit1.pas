...
var
  ...
  time:int64;
  not_found,first,Res, bTest, use_stdin, use_stdout, too_big:boolean;
...
if not(use_stdin and (GetFileSize(s+'.in')>1024)) then begin
	...
	s:='Размер файла теста #'+inttostr(t)+' превышает 1024 Б (данные не могут быть переданы тестируемой программе). Тестирование остановлено.';
	too_big:=true;
	numb:=99999;
	error:=8;
	Application.MessageBox(pchar(s),'Ошибка',mb_iconerror);
...
var s,st,file_text,error_str:string; re:cardinal; er,i,count,k,tim,got:integer; 
		report_add:ansistring; F,g:textfile; ch:char;
		t1,t2,t3,t4:_filetime;
begin
timer1.Enabled:=false;
timer2.Enabled:=false;
file_text:='';
...
if error<>3 then begin
	GetProcessTimes(pi.hProcess,t1,t2,t3,t4);
	time:=t3.dwLowDateTime div 1000;
end else time:=0;
got:=0;
if (error=3) and (time<500) and (timer1.Interval=1) then error:=0;
if (not not_found) and (not too_big) then begin
...
if error<>3 then GetExitCodeProcess(pi.hProcess,re) else re:=0;
...
try
  if (not(fileexists(extractfilepath(application.ExeName)+'tests\checker.exe'))) or (not(fileexists(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.in'))) or (not(fileexists(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.out')))  then er:=5 else begin
    ...
    tim:=0;
		chdir(extractfilepath(application.exename)+'tests\');
		outp:=TStringList.Create;
		ExecConsoleApp('checker.exe test.in test.out output.txt check.log',outp);
		sleep(400);
...
if (er<>3) and (er<>4) and (er<>5) then begin
  if time<1 then time:=random(300)+200;
...
if checkbox3.Checked=true then begin
  ...
  if (er<>3) and (er<>5) then report_add:=report_add+inttostr(time)+' ms' else report_add:=report_add+'Killed';
	...
	assignfile(f,extractfilepath(application.exename)+'tests\'+inttostr(numb)+'.in');
	reset(f);
	file_text:='';
	i:=0;
	while (i<255) and (not eof(F)) do begin
		inc(i);
		read(f,ch);
		file_text:=file_text+ch;
	end;       
	if i=255 then file_text:=file_text+'...';
	closefile(f);
	report_add:=report_add+file_text+'<o:p></o:p></span></p></td><td class="first"><p class="MsoNormal" align="center"><span class="second">';
	assignfile(f,extractfilepath(application.exename)+'tests\'+inttostr(numb)+'.out');
	reset(f);
	file_text:='';
	i:=0;
	while (i<255) and (not eof(F)) do begin
  ...
  file_text:='';
	if (fileexists(extractfilepath(application.exename)+'tests\'+edit2.text) or use_stdout) and (er<>2) and (er<>3) then begin
		...
		if not fileexists(extractfilepath(application.exename)+'tests\output.txt') then begin
			i:=filecreate(extractfilepath(application.exename)+'tests\output.txt');
			fileclose(i);
		end; 
		memo6.Lines.SaveToFile(extractfilepath(application.exename)+'tests\output.txt');
		assignfile(f,extractfilepath(application.exename)+'tests\output.txt');
		reset(f);
		i:=0;
		while (i<255) and (not eof(f)) do begin
			inc(i);
		...
		if i=255 then file_text:=file_text+'...';
		closefile(f);
		deletefile(extractfilepath(application.exename)+'tests\output.txt');
  end;
...
if (numb>spinedit2.Value) and (spinedit2.Value<>0) then  begin
  ...
  if checkbox3.Checked=true then begin
    ...
    if not_found and (length(label5.caption)=4) then file_text:='Tests has not been found!' else
