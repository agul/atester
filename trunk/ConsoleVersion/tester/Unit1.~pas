...
var Form1: TForm1;
    ...
    hPipeOutputRead,hPipeOutputWrite,hPipeErrorsRead,hPipeErrorsWrite: THANDLE;
...
var s,st,file_text,error_str:string; er,i,counter,j,k,tim,got,show:longint; report_add:ansistring; F,g:textfile;
begin
...
if fileexists(extractfilepath(application.exename)+'tests\'+output) then begin
  memo4.lines.LoadFromFile(extractfilepath(application.exename)+'tests\'+output);
  ...
  assignfile(f,s);
  reset(f);
  assignfile(g,extractfilepath(application.exename)+'tests\'+output);
  reset(g);
...
try
  if (not(fileexists(extractfilepath(application.ExeName)+'tests\'+checkername))) or (not(fileexists(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.in'))) or (not(fileexists(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.out')))  then er:=5 else begin
    memo7.Lines.LoadFromFile(extractfilepath(application.ExeName)+'tests\'+output);
    if not(fileexists(extractfilepath(application.ExeName)+'tests\'+ProgramOutput)) then begin
      k:=filecreate(extractfilepath(application.ExeName)+'tests\'+ProgramOutput);
      fileclose(k);
    end;
    if not(fileexists(extractfilepath(application.ExeName)+'tests\'+CheckerInput)) then begin
      k:=filecreate(extractfilepath(application.ExeName)+'tests\'+CheckerInput);
      fileclose(k);
    end;
    if not(fileexists(extractfilepath(application.ExeName)+'tests\'+CheckerOutput)) then begin
      k:=filecreate(extractfilepath(application.ExeName)+'tests\'+CheckerOutput);
      fileclose(k);
    end;
    if not(fileexists(extractfilepath(application.ExeName)+'tests\'+CheckerLog)) then begin
      k:=filecreate(extractfilepath(application.ExeName)+'tests\'+CheckerLog);
      fileclose(k);
    end;
    memo7.Lines.SaveToFile(extractfilepath(application.ExeName)+'tests\'+ProgramOutput);
    memo7.Lines.LoadFromFile(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.in');
    memo7.Lines.SaveToFile(extractfilepath(application.ExeName)+'tests\'+CheckerInput);
    memo7.Lines.LoadFromFile(extractfilepath(application.ExeName)+'tests\'+inttostr(numb)+'.out');
    memo7.Lines.SaveToFile(extractfilepath(application.ExeName)+'tests\'+CheckerOutput);
    tim:=0;
    chdir(extractfilepath(application.exename)+'tests\');
    show:=sw_hide;
    ShellExecute(random(1000), 'open', checkername, nil,nil,show);
    sleep(100);
    ...
    if fileexists(extractfilepath(application.exename)+'tests\'+CheckerLog)=false then er:=6 else begin
      memo7.Lines.LoadFromFile(extractfilepath(application.ExeName)+'tests\'+CheckerLog);
      if UpperCase(memo7.lines.strings[0])='OK' then er:=0 else
      if UpperCase(memo7.lines.strings[0])='WA' then er:=1 else er:=6;
    end;
    if fileexists(extractfilepath(application.exename)+'tests\'+CheckerLog) then DeleteFile(extractfilepath(application.exename)+'tests\'+CheckerLog);
  end;
except
  er:=6;
end;
...
if reportneed then begin
  ...
  report_add:=report_add+file_text+'<o:p></o:p></span></p></td><td class="first"><p class="MsoNormal" align="center"><span class="second">';
  file_text:='';
  if fileexists(extractfilepath(application.exename)+'tests\'+output) and (er<>2) and (er<>3) then begin
    memo6.lines.loadfromfile(extractfilepath(application.exename)+'tests\'+output);
    for i:=0 to memo6.lines.count-1 do file_text:=file_text+memo6.lines.strings[i]+'<br>';
  end;
...
deletefile(extractfilepath(application.exename)+'tests\'+input);
deletefile(extractfilepath(application.exename)+'tests\'+output);
inc(numb);
end;
...
procedure tform1.ExecConsoleApp(CommandLine: AnsiString; Output: TStringList; Errors: TStringList);
begin
...
if fileexists(s+'.in') and fileexists(s+'.out') then begin
  ...
  not_found:=false;
  error:=0;
  ExecConsoleApp(ProgramName,OutP,ErrorP);
end else begin
